...
1. Download the _latest stable Keptn version_ from [GitHub](https://github.com/keptn/keptn/releases), unpack it and move it to `/usr/local/bin/keptn`.
...
- _Linux / macOS_: Add executable permissions (`chmod +x keptn`), and move it to the desired destination (e.g. `mv keptn /usr/local/bin/keptn`)

- _Windows_: move/copy the executable to the desired folder and, optionally, add the executable to the PATH environment variable for a more convenient experience.
